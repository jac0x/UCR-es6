...
var BoardLayer = cc.Layer.extend({
    ...
    place: function place() {
        ...
        if (!this.getParent().canLayer.place()) {
            showMessageBoxOkCancel(this, "Game over...", "Completed puzzle in " + this.moves + " moves...", this.onReplay, null, "PLAY AGAIN", "CANCEL");
            setHighscore(this.moves);
        }
    },
    onBoard: function onBoard(sender, type) {
            ...
            for (i = 0; i < this.boards.length; i++) {
                if (this.boards[i].checkbox == sender) {
                    if (Math.floor(this.boards[i].answer / 3) == Math.floor(selectedTriple / 3)) {
                        ...
                        showMessageBoxOkCancel(this, "Try again", "That is an incorrect place for the set.", null, this.onNewTry, "TRY AGAIN", "CANCEL");
                    }
                    break;
                }
            }
        } else {
            showMessageBoxOK(this, "Invalid move", "You must select 3 cards before playing on the circle.", null);
            sender.setSelected(!sender.isSelected());
        }
    },
    onNewTry: function onNewTry() {
        var container = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this;
...
var CanLayer = cc.Layer.extend({
    ...
    onRefresh: function onRefresh(sender, type) {
        if (type != ccui.Widget.TOUCH_ENDED) return;
        this.refresh();
    },
    refresh: function refresh() {
        ...
        for (i = 0; i < con.boardCount; i++) {
            if (this.boardState[i] == con.canSelected) this.boardState[i] = con.canNormal;
        }var avails = [];
        // get available triples
        for (i = 0; i < con.boardCount; i += con.boardStep) {
            if (this.boardState[i] == con.canNormal) avails.push(i);
        } // finished
    ...
    checkSet: function checkSet() {
        var sels = [];
        for (i = 0; i < this.candidates.length; i++) {
            if (this.candidates[i].selected) sels.push(this.candidates[i].value);
        }if (sels.length != con.boardStep) return false;
        ...
        if (sels[0] % 3 == 0 && sels[1] == sels[0] + 1 && sels[2] == sels[0] + 2) return true;
        return false;
    },
    ...
    onCandidate: function onCandidate(sender, type) {
        if (type != ccui.Widget.TOUCH_ENDED) return;
        if (ready || dialogOpen) {
            sender.setSelected(!sender.isSelected());
            return;
        ...
        for (i = 0; i < this.candidates.length; i++) {
            if (sender == this.candidates[i].checkbox) {
                ...
                if (!sender.isSelected()) {
                    ...
                    if (counter == 3) {
                        if (this.checkSet()) showMessageBoxOK(this, "That's a set", "Now touch the correct place on the circle to play the set.", this.getReady);else showMessageBoxOK(this, "Try again", "Those 3 cards do not form a set.", this.deselectAll);
                    }
                } else {
                    counter--;
                    this.candidates[i].selected = false;
...
var showMessageBoxOK = function showMessageBoxOK(parent, title, message, callback) {
    dialogOpen = true;
    ...
    var btnOK = cc.MenuItemLabel.create(btnText, function () {
        dialogOpen = false;
...
var showMessageBoxOkCancel = function showMessageBoxOkCancel(parent, title, message, OkCallback, CancelCallback, okText, cancelText) {
    dialogOpen = true;
    ...
    var btnOk = cc.MenuItemLabel.create(btnOkText, function () {
        dialogOpen = false;
        layBackground.removeAllChildren(true);
        layBackground.removeFromParent(true);
        if (OkCallback) OkCallback(parent);
    }, this);
    ...
    var btnCancel = cc.MenuItemLabel.create(btnCancelText, function () {
        dialogOpen = false;
        layBackground.removeAllChildren(true);
        layBackground.removeFromParent(true);
        if (CancelCallback) CancelCallback(parent);
    }, this);
